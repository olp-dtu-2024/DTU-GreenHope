...
import { GateConfig, IResponsePaymentTransaction, Transaction } from '../transaction.i'
...
export abstract class Gate {
  ...
  isExists(payment: Transaction, paymentFromDb: { transactionId: string }[]): boolean {
    return paymentFromDb.some((el) => el.transactionId == payment.transaction_id)
  }
  ...
  replaceDateTodayAndNoTime = (date: Date): Date => {
    ...
    const dateNoTime = dateMoment.get('hour') == 0 && dateMoment.get('minute') == 0 && dateMoment.get('second') == 0
